...
const AddInventory = () => {
      ...
      try {
        ...
        if (!formData.institution_id && institutionList.length === 1) {
          setFormData((currentData) => ({
            ...currentData,
            institution_id: institutionList[0].id,
          }));
        }
      } catch (apiError) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch(INVENTORY_CREATE, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem(TOKEN_KEY)}`,
        },
      ...
      if (!response.ok) {
        setError(data.message || "Inventory add failed");
        return;
      }
            ...
            <div className="max-w-[720px] mx-auto w-full flex flex-col gap-6 md:gap-8">
              <div className="text-left">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Create a new institution inventory item
                </p>
              </div>
              ...
              <div className="grid gap-4">
                ...
                <InventoryForm
                  formData={formData}
                  buttonText={loading ? "Adding..." : "Add Inventory"}
