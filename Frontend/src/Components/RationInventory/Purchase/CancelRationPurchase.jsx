...
import { RATION_PURCHASE_CANCEL, TOKEN_KEY } from "../../../Utils/Constants";

const CancelRationPurchase = ({ purchase, onClose, onSuccess }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      const response = await fetch(RATION_PURCHASE_CANCEL, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        setError(data.message || "Failed to cancel purchase");
        return;
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 px-4 text-left">
      <div className="w-full max-w-md rounded-2xl border border-slate-100 bg-white p-6 shadow-xl flex flex-col gap-4">
        <div>
          <h2 className="text-xl font-black text-slate-800">Cancel Purchase Order</h2>
          <p className="mt-2 text-sm text-slate-500">
            Are you sure you want to cancel completed purchase order{" "}
            <span className="font-bold text-slate-700">{purchase.purchase_number}</span>?
            This will reverse the stock transaction logs and mark the purchase cancelled.
          </p>
        </div>
...
export default CancelRationPurchase;
