...
const EditRationItem = () => {
    ...
    const loadItemDetails = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          item_image_file: null,
          minimum_stock: String(item.minimum_stock !== null ? item.minimum_stock : "0"),
          maximum_stock: item.maximum_stock !== null ? String(item.maximum_stock) : "",
          reorder_quantity: item.reorder_quantity !== null ? String(item.reorder_quantity) : "",
          default_purchase_price: String(item.default_purchase_price !== null ? item.default_purchase_price : "0.00"),
          gst_percentage: String(item.gst_percentage !== null ? item.gst_percentage : "0"),
  ...
  // Load categories & units once institution_id is available
  useEffect(() => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      submitData.append("id", String(id));
      ...
      const response = await fetch(RATION_ITEM_EDIT, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem(TOKEN_KEY)}`,
        },
      ...
      if (!response.ok) {
        setError(data.message || "Failed to update ration item");
        setToast({ message: data.message || "Failed to update ration item", type: "error" });
        return;
      }

      setToast({ message: "Ration item updated successfully", type: "success" });
      setTimeout(() => {
    ...
    <div className="flex h-screen overflow-hidden bg-slate-50">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        <main className="flex-1 p-6 flex flex-col gap-6">
          <div className="flex items-center justify-between pb-4 border-b border-slate-200/60">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-xs font-semibold text-slate-500">
                Update details for ration item: {formData.sku_id}
              </p>
            </div>
          </div>
          ...
          {loadingItem || loadingDropdowns ? (
            <PageLoader message="Loading ration item details..." />
          ) : (
            <RationItemForm
              formData={formData}
