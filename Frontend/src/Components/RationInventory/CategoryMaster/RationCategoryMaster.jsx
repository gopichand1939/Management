...
import DeleteRationCategory from "./DeleteRationCategory";
import {
  RATION_CATEGORY_DELETE,
  RATION_CATEGORY_LIST,
...
const routePath = "/ration-inventory/category-master";
...
const RationCategoryMaster = () => {
  ...
  const getCategoryList = async (page = currentPage) => {
    ...
    try {
      const response = await fetch(RATION_CATEGORY_LIST, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem(TOKEN_KEY)}`,
          "Content-Type": "application/json",
      ...
      if (!response.ok) {
        setError(data.message || "Ration category list failed");
        return;
      }
      ...
      setError(apiError.message || "Ration category list failed");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredCategories = useMemo(() => {
    ...
    return categories.filter((category) => {
      return (
        ...
        category.category_code?.toLowerCase().includes(term) ||
  ...
  const tableData = filteredCategories.map((category, index) => {
    return {
      ...
      created_date: formatDate(category.created_at),
      status: <StatusBadge label={category.status || "active"} />,
    };
  });
  ...
  const handleDelete = async () => {
    ...
    setActionLoadingId(deleteCategory.id);
    setError("");

    try {
      const response = await fetch(RATION_CATEGORY_DELETE, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem(TOKEN_KEY)}`,
          "Content-Type": "application/json",
      ...
      if (!response.ok) {
        const message = data.message || "Category delete failed";
        setError(message);
        setToast({ message, type: "error" });
        return;
      }

      setToast({ message: "Ration category deleted successfully", type: "success" });
      setDeleteCategory(null);
      await getCategoryList();
    } catch (apiError) {
      const message = apiError.message || "Category delete failed";
      setError(message);
      setToast({ message, type: "error" });
    } finally {
  ...
  const renderActions = (category) => {
    ...
    return (
      <div className="flex flex-wrap items-center gap-2">
        <ActionPopOver
          onView={canView ? () => navigate(`${routePath}/view/${category.id}`, { state: { institution_id: instId } }) : null}
          onEdit={canEdit ? () => navigate(`${routePath}/edit/${category.id}`, { state: { institution_id: instId } }) : null}
          onDelete={canDelete ? () => setDeleteCategory(category) : null}
        />
      </div>
    );
  };
  ...
  return (
    <div className="grid min-h-screen grid-cols-1 bg-slate-50 lg:grid-cols-[270px_minmax(0,1fr)]">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 text-left">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Manage ration category records
                  </p>
                </div>
      ...
      <DeleteRationCategory
        ...
        loading={actionLoadingId === deleteCategory?.id}
...
export default RationCategoryMaster;
