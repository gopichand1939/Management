...
const AddRationCategory = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await fetch(RATION_CATEGORY_CREATE, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem(TOKEN_KEY)}`,
          "Content-Type": "application/json",
      ...
      navigate("/ration-inventory/category-master", {
        state: { toastMessage: "Ration category created successfully", toastType: "success" }
      });
    } catch (apiError) {
            ...
            <div className="max-w-[720px] mx-auto w-full flex flex-col gap-6">
              <div className="text-left">
                ...
                <p className="text-sm text-slate-500 mt-1">
                  Create a new ration category master record
                </p>
              </div>
...
export default AddRationCategory;
