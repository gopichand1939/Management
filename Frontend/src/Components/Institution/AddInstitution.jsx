...
const AddInstitution = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    status: "active",
    logo: "",
    floors: [createDefaultFloor(0)], // default starting with 1 floor
  });

  // Modal / Drawer Selection states
  const [selectedFloorIdx, setSelectedFloorIdx] = useState(0);
  const [isRoomModalOpen, setIsRoomModalOpen] = useState(false);
  const [editingRoomIdx, setEditingRoomIdx] = useState(null); // idx or null
  ...
  // Aggregate totals dynamically for preview steps
  const stats = useMemo(() => {
  ...
  // Field change handler for Step 1 Info
  ...
  const handleStepClick = (stepIdx) => {
    // Enable jump backwards unconditionally, but validate jumps forward
  ...
  // Step 2 Floor Functions
  const handleAddFloor = () => {
    const nextIdx = formData.floors.length;
  ...
  // Step 4 Bed Configuration Drawer trigger
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(INSTITUTION_CREATE, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        setError(data.message || "PG Onboarding failed");
        return;
      }

      navigate("/institutions");
    } catch (apiError) {
      setError(apiError.message || "Something went wrong saving details");
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex-1 w-full pt-7 lg:pt-8 pb-8 px-6 md:px-8">
            <div className="max-w-4xl mx-auto w-full flex flex-col gap-6">
              ...
              <div className="flex items-center gap-3 border-b border-slate-100 pb-4 text-left">
                ...
                <div>
                  <h1 className="text-2xl font-black text-slate-800 tracking-tight">Onboard New PG</h1>
                  <p className="text-xs text-slate-400 font-bold mt-0.5">Define your PG Profile, levels, rooms, and visual bed templates.</p>
                </div>
              </div>
                            ...
                            >
                              ...
                              <div className="flex items-center justify-between border-b border-slate-50 pb-2 mt-1">
                                ...
                                <div className="flex items-center gap-1.5">
                                  ...
                                  <span className="text-[9px] bg-slate-50 border border-slate-100 px-2 py-0.5 rounded-lg text-slate-550 font-black uppercase tracking-wider">
              ...
              <div className="flex items-center justify-between border-t border-slate-200 pt-5">
                ...
                {activeStep < steps.length - 1 ? (
                  ...
                  >
                    <ShieldCheck size={14} />
                    <span>{loading ? "Onboarding PG..." : "Confirm & Save"}</span>
                  </button>
                )}
              </div>
            </div>
              ...
              <div className="mt-5 flex flex-col gap-4">
                ...
                <div className="grid gap-1">
                  ...
                  <select
                    ...
                    className="h-11 rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 outline-none focus:border-orange-500/50 focus:ring-4 focus:ring-orange-500/10 transition-all cursor-pointer"
                  >
