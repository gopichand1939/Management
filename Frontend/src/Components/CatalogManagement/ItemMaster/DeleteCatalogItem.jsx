...
import { CATALOG_ITEM_DELETE, TOKEN_KEY } from "../../../Utils/Constants";
...
const DeleteCatalogItem = ({
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(CATALOG_ITEM_DELETE, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        const msg = data.message || "Failed to delete item";
        setError(msg);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-xs">
      <div className="w-full max-w-[480px] overflow-hidden rounded-2xl border border-slate-100 bg-white p-6 shadow-xl text-left">
        <div className="flex items-center gap-3.5 mb-4">
          ...
          <div>
            <h3 className="text-base font-bold text-slate-950">Delete Catalog Item</h3>
            <p className="text-xs text-slate-500 font-semibold mt-0.5">This action is permanent</p>
          </div>
        </div>
        ...
        <p className="text-sm text-slate-600 font-medium mb-6 leading-relaxed">
          ...
          {item?.sku ? ` (SKU: ${item.sku})` : ""}? This will permanently remove the item from the catalog template database.
        </p>
