...
const uploadDirectory = path.join(__dirname, "..", "uploads", "inventory");
...
const inventoryUpload = upload.single("item_photo");

const handleInventoryUpload = (req, res, next) => {
    inventoryUpload(req, res, async (error) => {
        if (error) {
        ...
        if (req.file) {
            try {
                req.file.cloudinaryUrl = await uploadToCloudinary(
                    req.file.path,
                    "inventory/item_photo"
                );
            } catch (uploadError) {
                console.error("Cloudinary upload error in inventory middleware:", uploadError);
