...
const RationStockIssueModel = {
    getNextStockIssueNumber: async (institutionId, client) => {
        ...
        // Find the maximum numeric suffix from existing stock issue numbers in this institution
        const maxNumQuery = `
            SELECT COALESCE(MAX(NULLIF(regexp_replace(issue_number, '[^0-9]', '', 'g'), '')::BIGINT), 0) AS max_num
            FROM ration_stock_issues
            WHERE institution_id = $1 AND issue_number ~* '^SI\\d+$'
        `;
        ...
        const query = `
            ...
            DO UPDATE SET
                last_number = GREATEST(ration_stock_issue_sequences.last_number + 1, EXCLUDED.last_number),
                updated_at = CURRENT_TIMESTAMP
            RETURNING last_number;
        `;
        const result = await executor.query(query, [institutionId, maxNumVal]);
